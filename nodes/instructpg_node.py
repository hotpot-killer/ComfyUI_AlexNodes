...
from .ImageFlow.ImageFlow import VisualTextModel as ImageFlow
...
class InstructPGStableDiffusionPipeline(DiffusionPipeline, StableDiffusionMixin):
    ...
    def INPUT_TYPES(cls):
        return {
            "required":{
                ...
                "image": ("IMAGE",),
                "steps": ("INT", {"default": 50, "min": 1, "max": 500}),
                "cfg": ("FLOAT", {"default": 7.0, "min": 1.0, "max": 100.0}),
        }
    }
    RETURN_TYPES=("IMAGE",) 
    # RETURN_NAMES:定义输出标签
    RETURN_NAMES=("RETURN IMAGE",)
    FUNCTION = "InstructPG"
    CATEGORY = "AlexNodes/InstrucgPG"
    ...
    def __init__(
        ...
        dataset = load_dataset(
             "json" , data_files="./hf_img_edit_preference.jsonl"
        )
    ...
    def train(
        ...
        text_embedding_optimization_steps: int = 10,
        model_fine_tuning_optimization_steps: int = 10,
        **kwargs,
    ):
        ...
        for _ in range(text_embedding_optimization_steps):
            with accelerator.accumulate(text_embeddings_optim):
                # Sample noise that we'll add to the latents
                noise = torch.randn(image_latents.shape).to(self.device)
                timesteps = torch.randint(800, (1,), device=self.device)
        ...
        # 3. Fine tuning the Unet using reward model
        self.preference_model = ImageFlow.load(
            "./pretrained_weights/imageflow_state_dict.pt",
            device=self.device,
        )
        ...
        for epoch in range(0, 10):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                with accelerator.accumulate(self.unet):
                    ...
                    ).pred_original_sample.to(latents_dtype)
